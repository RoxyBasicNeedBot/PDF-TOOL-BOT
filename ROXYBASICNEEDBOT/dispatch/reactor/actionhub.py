...
@RoxyBot.on_callback_query(filters.regex("^pdf"))
async def _pdf(bot, callbackQuery):
    try:
        ...
        if data == "more":
            ...
            tTXT, tBTN = await util.translate(
                button="BUTTONS['compress']", order=1111, lang_code=lang_code
            )
...
@RoxyBot.on_callback_query(filters.regex("beta"))
...
@RoxyBot.on_callback_query(filters.regex("^aio"))
    ...
    try:
        ...
        if not reply_msg or not reply_msg.document:
            return await callbackQuery.message.reply_text(
                "❌ Original file not found. Please send the document again.", quote=True
            )
        ...
        # encrypted/non encrypted input pdf file
            ...
            return await callbackQuery.message.edit(
                text=tTXT["passMSG"].format(
                    reply_msg.document.file_name,  # password 300 char limit
                    await render.gSF(
                        reply_msg.document.file_size
                    ),
                    input_str.text[:50]
